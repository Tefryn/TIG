...
TEST(Sha1, EmptyString) {
  std::string in = "";
  auto hash = sha1({reinterpret_cast<const uint8_t *>(in.data()), in.size()});
...
TEST(Sha1, DifferentInputsDifferentHashes) {
  std::string a = "foo";
  std::string b = "bar";
  auto ha = sha1({reinterpret_cast<const uint8_t *>(a.data()), a.size()});
  auto hb = sha1({reinterpret_cast<const uint8_t *>(b.data()), b.size()});
  EXPECT_NE(ha, hb);
}

TEST(Sha1, DeterministicSameInputSameHash) {
  std::string in = "hello, world";
  auto h1 = sha1({reinterpret_cast<const uint8_t *>(in.data()), in.size()});
  auto h2 = sha1({reinterpret_cast<const uint8_t *>(in.data()), in.size()});
  EXPECT_EQ(h1, h2);
}
